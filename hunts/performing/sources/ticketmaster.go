...
// Ticketmaster fetches performing arts events from the Ticketmaster Discovery API.
type Ticketmaster struct {
	apiKey     string
	httpClient *http.Client
}

// NewTicketmaster creates a Ticketmaster source for performing arts.
...
func (t *Ticketmaster) Name() string { return "ticketmaster-performing" }
...
func eventToRawItem(e event) *core.RawItem {
	...
	for _, cls := range e.Classifications {
		if cls.Genre.Name != "" && cls.Genre.Name != "Undefined" {
			item.Subtitle = cls.Genre.Name + " — " + item.Subtitle
			break
		}
	}
...
func resolveShowTime(e event) string {
	if e.Dates.Start.LocalDate != "" && e.Dates.Start.LocalTime != "" && e.Dates.Timezone != "" {
		...
		if err == nil {
			...
			t, err := time.ParseInLocation(localTimeLayout, localStr, loc)
			if err == nil {
