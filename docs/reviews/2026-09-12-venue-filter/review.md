# Venue filter

Venue dropdown combines with the existing rating filter and sort order. Its options come from current, unfiltered cards. URL state survives refresh and form submissions; changing hunt starts that hunt's default view. Selecting All venues removes only venue restriction; Clear filters in the empty state clears both filters.

Uses the existing address-checked Comedy Works Downtown alias, preserving South as a separate venue. Other venue keys retain their normalized address, and duplicate names display addresses to disambiguate source records. No database mutations or changes to recommendation scoring, scans, or notification preferences.

Web integration tests verify combined filtering, downtown alias membership, exclusion of South, redirects, and recovery for a venue with no current cards. Full Go tests and vet passed. A focused read-only reviewer found no actionable defect; the independent Gemini review confirmed the filtering and state behavior and found a count mismatch plus fragile address-label parsing. Both were corrected with regression tests. An empty-label guard was also added. The suggestion to move the established alias display mapping upstream was deferred because changing ingestion is outside this browsing feature.

Browser interaction verification on an enriched production copy: Downtown selected 14 cards, adding 8+ selected 5, date ordering put Ralph Barbosa before Mark Normand; changing to Red Rocks selected 1 and returning preserved both score and date. Desktop and 390x844 mobile screenshots attached. The mobile venue control occupies a full-width row and the page has no horizontal overflow. This is a small extension of the previously reviewed controls; visual review used direct affected-render inspection. Final preview enabled all four hunt renderers with placeholder credentials and no scan or notification execution. Buell Theatre selected 10 performing-arts cards, all with the expected venue. An unavailable venue showed the reset link; clicking it cleared the restriction. Final images show Downtown with All (14).

Screenshots: desktop.png and mobile.png.
